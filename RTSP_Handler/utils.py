...
# Helper to safely quote table identifiers (e.g., DB or table names with hyphens)
def _quote_identifier(path: str) -> str:
    """
    Quote a potentially dotted identifier like db.table using backticks around each part.
...
def upload_image_to_s3(file_name: str) -> bool:
    try:
        if os.path.exists(file_name):
            ...
            print("frame path not found")
    except Exception as e:
        print(f"Exception in S3 upload : {e}")
...
def get_cameras_from_db() -> dict:
    ...
    try:
        ...
        print(f"\n *** table_ref :: {table_ref} ***")
        ul = _quote_identifier('user_location')
        loc = _quote_identifier('location')
        usr = _quote_identifier('user')
        query = f"""
            SELECT camera_rtsp.* FROM {table_ref} AS camera_rtsp
            INNER JOIN {ul} ON {ul}.location_id = camera_rtsp.location_id
            INNER JOIN {loc} ON {loc}.id = {ul}.location_id
            INNER JOIN {usr} ON {usr}.id = {ul}.user_id
            WHERE {usr}.id = %s;
        """
        print(f"\n *** query :: {query} ***")
        ...
        data_dict = {
            data_item["id"]: {
                ...
                "camera_rtsp_url": data_item["rtsp_url"],
                "camera_status": data_item["status"],
                "camera_fps": data_item["process_fps"],
                "scale": int(data_item["camera_resolution"].split(":")[0]),
            }
            for data_item in data
        }
        return data_dict
...
def update_db_statuses(camera_id: int, status: bool) -> None:
    try:
        cursor = db_mysql.cursor()
        cursor.execute(
            f"""
        UPDATE deployment_job_rtsp_manager 
        SET is_active={status}, is_processing={status}, updated_date='{datetime.datetime.utcnow().replace(microsecond=0)}' 
        WHERE id={camera_id}"""
        )
        db_mysql.commit()
        cursor.close()
    except Exception as e:
        logger.error(f"Exception while updating db_statuses : {e}")
...
def set_notifier():
    ...
    try:
        ...
        notifier.loop()
        logger.info("In Function set_notifier :: notifier stop")
    except Exception as e:
...
def get_command_for_videos(camera_id: str, config: dict) -> list:
    ...
    return [
        ...
        f"{settings.FRAMES_ROOT_DIR}{os.sep}raw_frames{os.sep}{str(camera_id)}/'video_%Y%m%d_%H%M%S.mp4'"
    ]
...
def create_directories() -> None:
    ...
    for camera_id, config in cam_config.items():
        if config.get("camera_status"):
            # e.g. /home/mihir/client/raw_frames/0
            dir_path = (
                    settings.FRAMES_ROOT_DIR
                    + os.sep
                    + "raw_frames"
...
def check_rtsp(rtsp: str, camera_id: int):
    ...
    if rtsp:
        ...
        if os.path.isfile(file_name):
            ...
            rtsp_status = False
        if settings.CAM_SOURCE == "db":
            update_db_statuses(camera_id=camera_id, status=rtsp_status)
        return rtsp_status
    else:
...
async def run_command_shell(command):
    ...
    if stderr:
        ...
        check_rtsp(
            ...
            camera_id=int(command.split("raw_frames/")[-1].split(os.sep)[0]),
        )
